...
import torch
from models.rt_detr_cross.track_utils.kalman_filter import KalmanFilter
from models.rt_detr_cross.track_utils.log import logger
from models import *
...
class JDETracker(object):
    def __init__(self, frame_rate=30):
        ...
        self.frame_id = 0
        self.buffer_size = int(frame_rate / 30.0 * 24)
        self.max_time_lost = self.buffer_size

        self.kalman_filter = KalmanFilter()

    def update(self, box_results, detr_det_logits, detr_pred_id_words):
        """
        ...
        removed_stracks = []
        box_results = box_results.numpy().astype(np.float32)
        detr_det_logits = detr_det_logits.numpy().astype(np.float32)
        ...
        detections = [STrack(STrack.tlbr_to_tlwh(tlbr), label, f.numpy().astype(np.float32), 30) for
                      (tlbr, label, f) in zip(box_results, detr_det_logits, detr_pred_id_words)]
