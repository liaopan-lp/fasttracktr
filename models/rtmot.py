...
from .rt_detr.rtdetr import build_rt_detr
    ...
    def __init__(self, config: dict):
        ...
        if self.only_detr is False:
            self.seq_decoder = SeqDecoder(
                ...
                training_num_id=config["NUM_ID_VOCABULARY"] if "TRAINING_NUM_ID" not in config else config[
                    "TRAINING_NUM_ID"],
                device=self.device,
        ...
        if detr_checkpoint_frames is not None:
            ...
            for batch_frames in batch_iterator(detr_checkpoint_frames, frames):
                ...
                if targets is None:
                    ...
                    _ = self.detr(frames, batch_frames, max_gt_num=max_gt_num)
                else:
            ...
            if targets is None:
                ...
                detr_outputs = self.detr(frames, targets, max_gt_num=max_gt_num)

        return detr_outputs


def build(config: dict):
    return RTMOT(config=config)
